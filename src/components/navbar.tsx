		...
		<nav className="w-full">
			<div className="flex flex-row justify-between px-4">
								...
								<motion.p
									...
									className={`${
										index === navLinks.length - 1
											? "bg-blue text-black px-4 py-2.5 rounded-full"
											: "text-gray hover:text-white pr-6"
									}`}
								>
									{link.name}
								</motion.p>
				...
				<motion.div
					...
					initial="close"
					className={`z-[5] flex flex-col gap-3 md:hidden py-4 ${
						isOpen ? "fixed z-[5] right-0 h-full bg-black" : ""
					}`}
				>
					{/* Dropdown Button */}
					<button
					...
					<div
						className={
							isOpen
								? "flex flex-col gap-3 px-2 text-center"
								: "hidden"
						}
					>
						{navLinks.map((link, index) => {
